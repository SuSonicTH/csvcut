...
pub const LineReader = struct {
    ...
    pub fn read_line(self: *LineReader) void {
        _ = self;
    }
    ...
    fn fill_buffer(self: *LineReader) !usize {
        ...
        const read = try self.reader.read(self.buffer[self.end..][0..self.read_size]);
        if (read == 0) {
            self.eof = true;
        }
        self.end += read;
...
test "init" {
    ...
    const path = try std.fs.realpath("src/test.csv", &path_buffer);
    ...
    try testing.expectEqual(30, line_reader.size);
